...
const std::string* Synth::Impl::getKeyLabel(int swNumber)
{
    auto it = keyLabelsMap_.find(swNumber);
    return (it == keyLabelsMap_.end()) ? nullptr : &keyLabels_[it->second].second;
}

void Synth::Impl::setKeyLabel(int swNumber, std::string name)
{
    auto it = keyLabelsMap_.find(swNumber);
    if (it != keyLabelsMap_.end())
        ...
        keyLabels_.emplace_back(swNumber, std::move(name));
        keyLabelsMap_[swNumber] = index;
    }
}
